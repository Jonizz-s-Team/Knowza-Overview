...
const KnowzaAIFlashCards = () => {
  ...
  const [activeTab, setActiveTab] = useState('decks'); // 'decks' | 'vocabulary'
  ...
  const [viewMode, setViewMode] = useState('review'); // 'review' or 'table'
  ...
  useEffect(() => {
    ...
    if (lastDaily !== todayStr && !decksLoading && selectedExamType) {
      startFlashGeneration("Kunlik Avto-Lug'at");
      apiService.autoGenerateDailyFlashcards(selectedExamType, 20)
        .then(() => {
          localStorage.setItem('knowza_fc_last_daily_date', todayStr);
          queryClient.invalidateQueries({ queryKey: ['flashcardDecks'] });
          finishFlashGeneration("Kunlik AI Lug'at");
        })
        .catch(e => {
  ...
  const goNextCard = useCallback(() => {
    ...
    }, 500); // Wait for the 500ms flip animation to fully finish
  }, [queryClient]);
  ...
  const { data: deckCardsData } = useQuery({
    queryKey: ['deckCards', activeDeck?.id || activeDeck?._id],
    queryFn: () => apiService.getFlashcardDeckCards(activeDeck.id || activeDeck._id),
    enabled: !!activeDeck,
  });
  const allDeckCards = deckCardsData?.data || deckCardsData || [];

  const { data: allUserCardsData } = useQuery({
    queryKey: ['allUserFlashcards'],
    queryFn: () => apiService.getAllUserFlashcards(),
  });
  const allUserCards = allUserCardsData?.data || allUserCardsData || [];

  const groupedUserCards = useMemo(() => {
    const groups = {};
    allUserCards.forEach(card => {
      const deckName = card.deck_title || t("To'plamsiz");
      if (!groups[deckName]) groups[deckName] = [];
      groups[deckName].push(card);
    });
    return groups;
  }, [allUserCards, t]);
  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-6 w-full mx-auto animate-in fade-in duration-500">
        ...
        {/* Main Tabs (Removed per user request to show everything together) */}
        ...
        {/* Global Vocabulary Table (Main Page) */}
        {!activeDeck && !reviewSummary && (
          <div className="w-full flex flex-col gap-8 mt-8 border-t border-[#e5e2e1] pt-8">
            <h2 className="text-[24px] leading-[32px] font-bold text-[#1c1b1b]">
              Your Vocabulary Knowledge Base
            </h2>
            {allUserCards.length > 0 ? (
              Object.entries(groupedUserCards).map(([deckTitle, cards]) => (
                <div key={deckTitle} className="w-full">
                  <h3 className="text-xl font-bold text-[#1c1b1b] mb-4 flex items-center gap-2">
                    <span className="material-symbols-outlined text-[#274ed5] text-[24px]">bookmark</span>
                    {deckTitle}
                  </h3>
                  <div className="bg-white rounded-3xl border border-[#e5e2e1] shadow-xs overflow-hidden w-full">
                    <div className="overflow-x-auto">
                      <table className="w-full text-left border-collapse min-w-[800px]">
                        <thead>
                          <tr className="bg-[#fcf9f8] border-b border-[#e5e2e1]">
                            <th className="p-4 text-[13px] font-bold text-[#747686] uppercase w-[20%]">Word</th>
                            <th className="p-4 text-[13px] font-bold text-[#747686] uppercase w-[20%]">Translation</th>
                            <th className="p-4 text-[13px] font-bold text-[#747686] uppercase w-[20%]">Definition</th>
                            <th className="p-4 text-[13px] font-bold text-[#747686] uppercase w-[30%]">Example</th>
                            <th className="p-4 text-[13px] font-bold text-[#747686] uppercase w-[10%] text-center">Mastery Level</th>
                          </tr>
                        </thead>
                        <tbody>
                          {cards.map((c, i) => {
                            // Determine mastery color
                            let masteryColor = "bg-red-50 text-red-600 border-red-100";
                            if (c.interval_days > 21) masteryColor = "bg-emerald-50 text-emerald-600 border-emerald-100";
                            else if (c.interval_days > 7) masteryColor = "bg-blue-50 text-[#274ed5] border-blue-100";
                            else if (c.interval_days > 1) masteryColor = "bg-amber-50 text-amber-600 border-amber-100";

                            return (
                              <tr key={c.id || i} className="border-b border-[#e5e2e1] hover:bg-[#fcf9f8] transition-colors">
                                <td className="p-4 align-middle">
                                  <span className="font-bold text-[#1c1b1b] text-[15px] block">{c.front}</span>
                                </td>
                                <td className="p-4 align-middle text-[14px] font-semibold text-[#274ed5]">{c.back}</td>
                                <td className="p-4 align-middle text-[13px] text-[#444654]">{c.hint || '-'}</td>
                                <td className="p-4 align-middle text-[13px] text-[#747686] italic">{c.example_sentence || '-'}</td>
                                <td className="p-4 align-middle text-center">
                                  <div className={`px-2 py-1 rounded-lg border text-[11px] font-bold whitespace-nowrap inline-flex ${masteryColor}`}>
                                    {c.is_mastered ? "Mastered" : (c.interval_days > 0 ? `${c.interval_days} days` : "New")}
                                  </div>
                                </td>
                              </tr>
                            );
              ...
              <div className="bg-white rounded-3xl border border-[#e5e2e1] p-12 text-center shadow-xs">
                <span className="material-symbols-outlined text-[48px] text-[#747686] mb-4 opacity-50">book</span>
                <p className="text-[16px] text-[#747686] font-medium">
            ...
            <div className="bg-white rounded-3xl w-full max-w-lg shadow-xl overflow-hidden animate-in fade-in zoom-in duration-200 border border-[#e5e2e1]">
              ...
              <form onSubmit={handleGenerate} className="p-6 space-y-5">
                ...
                <div>
                  ...
                  />
                  <div className="text-[11px] text-[#747686] mt-1 font-bold">
                    Maksimal 100 ta
                  </div>
                </div>
